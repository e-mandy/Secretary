        ...
        <section class="bg-white p-6 rounded-lg shadow mb-10">
            <div class="flex justify-between mb-6">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-2xl font-bold">{{ $professeur->lastname }} {{ $professeur->firstname }}</h2>
                        <span class="font-medium text-gray-500">{{ $professeur->specialite }}</span>
                    </div>
                </div>
                <div>
                    <p>Ajouté le {{ $professeur->prof_created_at_low_format }}</p>
                </div>
            </div>
            <div class="grid grid-cols-2 gap-y-3">
                ...
                <p class="flex items-center gap-3">
                    ...
                    {{ $professeur->nbr_docs }} document(s)
                </p>
                <p class="flex items-center gap-3">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
        ...
        <section>
            <ul class="onglets flex mb-8">
                ...
                <li id="documents" class="w-1/2 text-center cursor-pointer py-1">
                    <a>Documents ({{ $professeur->nbr_docs }})</a>
                </li>
            </ul>
            ...
            <div class="contains hidden bg-white p-5 rounded-lg" id="docs">
                <div class="mb-5">
                    ...
                    <p class="text-gray-500 font-medium">{{ $professeur->nbr_docs }} documents(s) enregistrés</p>
                </div>
                <table class="w-full">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        @foreach($professeur->documents as $document)
                            <tr class="border-t-1 border-gray-300">
                                <td class="w-1/3">{{ $document->nom }}</td>
                                <td>
                                    <span class="border border-gray-300 px-2 rounded-xl">{{ $document->type->nom }}</span>
                                </td>
                                <td>{{ $document->date_created_at }}</td>
                                <td class="flex gap-3 items-center">
                                    <a class="block border border-gray-300 p-1 rounded hover:bg-gray-200" href="{{ route('admin.documents.download', $document) }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
